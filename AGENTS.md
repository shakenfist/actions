...
**Documentation links** follow two rules that pull against "just use
relative links", and both are enforced by
`tests/test_documentation_links.py`. Every link in the top-level
`README.md` must be **absolute**: it is rendered off the landing page,
where a relative target resolves against the wrong base. A relative
link inside `docs/` must **stay inside** `docs/`; anything pointing
above it must be absolute, because `docs/` is republished on
shakenfist.com without the tree above it, so an escaping link renders
correctly on GitHub and 404s there.
