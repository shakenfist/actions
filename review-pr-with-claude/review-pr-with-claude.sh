...
if [ "${diff_fetch_ok}" = "false" ]; then
    ...
    if echo "${diff_err}" | \
        ...
        # Diff too big for the API. Post a polite "review
        # skipped" comment so PR readers know why the bot didn't
        # weigh in, and exit success so the workflow step is
        # green. A green check here means "the reviewer ran to a
        # known endpoint," not "the PR is reviewed."
        skip_comment="${output_dir}/skip-comment.md"
        cat > "${skip_comment}" << 'COMMENT_EOF'
## :robot: Automated review skipped
...
This message is generated by the reviewer's "diff too large"
handler; the workflow step exited successfully so it doesn't
block the merge queue.
COMMENT_EOF
        gh pr comment "${pr_number}" --body-file "${skip_comment}" \
            || echo "Warning: failed to post skip comment"
        ci_output "review_skipped" "diff_too_large"
        ci_output "review_posted" "false"
        echo
        echo "========================================"
        echo "PR review skipped (diff too large)"
        echo "========================================"
        exit 0
    fi
